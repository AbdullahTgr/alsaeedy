    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.videos.update", [$video->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('channel') ? 'is-invalid' : '' }}" name="channel_id" id="channel_id" required>
                    @foreach($channels as $id => $channel)
                        <option value="{{ $id }}" {{ ($video->channel ? $video->channel->id : old('channel_id')) == $id ? 'selected' : '' }}>{{ $channel }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('youtube_embed') ? 'is-invalid' : '' }}" name="youtube_embed" id="youtube_embed" required>{{ old('youtube_embed', $video->youtube_embed) }}</textarea>
            ...
            <div class="form-group">
                <label for="tags">{{ trans('cruds.video.fields.tags') }}</label>
                <textarea class="form-control {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags" id="tags">{{ old('tags', $video->tags) }}</textarea>
                @if($errors->has('tags'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tags') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.video.fields.tags_helper') }}</span>
            </div>
            <div class="form-group">
